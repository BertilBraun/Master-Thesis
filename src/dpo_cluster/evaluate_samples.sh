...
#SBATCH --partition=gpu_4_a100 # TODO gpu_4_a100             # mby GPU queue for the resource allocation.
#SBATCH --time=05:00:00  # TODO 3h?                  # wall-clock time limit
#SBATCH --mem=100000                       # memory per node
#SBATCH --nodes=1                          # number of nodes to be used
#SBATCH --cpus-per-task=4                  # number of CPUs required per MPI task
#SBATCH --ntasks-per-node=1                # maximum count of tasks per node
#SBATCH --mail-type=ALL                    # Notify user by email when certain event types occur.
#SBATCH --gres=gpu:4 # TODO 4
